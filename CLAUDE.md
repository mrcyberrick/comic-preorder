...
| F132 | **Medium** — a title restricted to a distributor allocation ratio (e.g. `1:10`) carries no signal at reservation time; customer only learns via the retrospective F117/F120 rejected badge. **Both distributors** — corrected same-day, Lunar carries the ratio in `variant_type` (562 rows, staging), not absent as first measured | Owner: `docs/order-restriction-alert-badge.md` (staging V1-V4 GREEN 2026-08-20 — migration applied, badge real-browser-verified via Playwright, 210/210 unit tests. **V5 moved up to 2026-08-21** — Rick running a real import that day. Production not requested) |
