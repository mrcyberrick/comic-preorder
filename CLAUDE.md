...
### PLANNED — Session B only; Session A is closed
...
- **Session B — surface and act** (client). **F113 pagination first**, then
  ...
  **Confirm Session A's four columns exist on both environments before
  starting** (they do, per above) — B renders what A produces.
