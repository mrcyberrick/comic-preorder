...
**Last completed work: F99 S3 ATTEMPTED and ROLLED BACK, 2026-09-01, same day as S2.** A real
...
**Two real, unanticipated problems, found live, not predicted by the plan or MailerSend's own
docs.** (1) API tokens are scoped to a specific domain ("server"), not the account — the existing
token died the moment `mrcyberrick.us` was removed (`Unauthenticated`), fixed by generating a new
Sending-access token. (2) **The one that forced the rollback:** MailerSend's send-time domain check
did not activate for 15–20+ minutes past full dashboard verification — confirmed for **both** the
subdomain and the exact bare verified domain (`noreply@pulllist.app` itself failed identically,
ruling out anything subdomain-specific), via the real error `#MS42207 — "The from.email domain must
be verified in your account to send emails."` Waited twice (2.5 min, then 10 more) with no change
before rolling back, per a decision point agreed with Rick in advance rather than guessed at in the
moment.
...
`pulllist.app`-scoped API token is now dead (harmless, safe to delete in MailerSend whenever
convenient). **Before attempting S3 again:** budget real slack past the observed 15–20 min floor
(the true ceiling is unknown); generate the new domain-scoped token as an explicit sub-step
immediately after adding the domain; consider asking MailerSend support about activation timing
*before* the next attempt rather than discovering it live in another outage window.
