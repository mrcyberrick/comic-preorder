...
### PLANNED — two execution sessions, A before B, not started

**Plan: `docs/order-export-followthrough-f110-f111-f112.md`** (written 2026-08-03,
planning session). Covers **F110**, **F111**, **F112(a)** and **F113**. Each
session has its own runbook, gates and completion criteria; **run `/handoff`
against that doc rather than starting from this summary.**

- **Session A — detect and record** (private scripts repo + four additive
  `catalog` columns). F110 withdrawal detection as an explicit cross-month
  set difference; F112(a) reads `InitialOrderDue` + `TitleNote`. Schema must
  land on **staging and production** before Session B promotes.
- **Session B — surface and act** (client). **F113 pagination first**, then
  F111's cross-month widening, F110's admin panel + My List flag + cancel
  exception, and the F112(b) copy correction. Spec 15 must be extended.

**A before B** because B renders what A produces — there is nothing to show
until A has run at least once.

**All three findings were filed 2026-08-03 from the PRH and Lunar vendor
documentation, read at Rick's request before extending the order-export
work — deliberately, to avoid designing against assumed distributor
behaviour. That read corrected one wrong fix direction (F110) and one wrong
severity assumption (F111) before either reached code. The planning session
then corrected two more things before code:** F110's *replacement* fix
direction was also wrong (it reused a function that matches zero rows — see
F66), and **F112(b)'s severity split was overruled by Rick outright.** Reading
the vendor docs was worth it; trusting them without asking the operator was
not.
...
- **PRH holds 12 copies of `75960621668000111` against 7 reservations**
  (MIDNIGHT X-MEN #1), FOC **2026-08-31**. The F101/F102 work now makes this
  *visible* — production's ledger records it and the By Distributor row reads
  as an over-order — but **the surplus itself is only fixable by calling PRH
  and adjusting the order down.** Reminder scheduled for 2026-08-24 (routine
  `trig_01D8pWAMP5uuLqqb62gDjGrY` + calendar event). See § 13 F102.
