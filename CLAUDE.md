...
**Last completed work:** **Production's real September import + F147 (severe) + F146 — both found
and fixed same session, 2026-08-28.** Rick ran `node import.js` for real against production
...
279/279. **Production's 519 false marks are NOT yet cleared** — the CLI session's own permission
classifier blocked the write (a bulk PATCH against production), correctly, by design. A local
one-off script (`clear-f147-withdrawn.js`, ids already captured for exact revertibility) is ready —
needs Rick to run `node clear-f147-withdrawn.js` from the scripts folder. 0 of the 519 have a passed
FOC, so the script will withhold none and clear all 519. **Maintenance Mode remains ON**, so this is
not urgent — no customer has seen any of the 519 marks. Full detail: `docs/technical-reference.md`
§ 13 F147.
...
|---|---|---|
| F147 | **High, code fixed same day 2026-08-28, data correction still owed** — F110's first-ever real run, on production, marked **519 of 1,571 open reservations (33%)** withdrawn — every one still inside its own ordering window (`foc_date` not yet passed; example BATMAN #14, FOC two weeks out). `narrowWithdrawalCandidates()` never checked FOC at all | **Code fixed, scripts repo `main` `e4f968d`** — now requires `foc_date` present and passed; 6 new tests incl. BATMAN #14's real shape, negative-control tested, 279/279 green. **Still owed: run `node clear-f147-withdrawn.js`** to clear the 519 (0 have a passed FOC, so all 519 will clear, none withheld) — blocked from CLI execution by the session's permission classifier, ready for Rick to run. **Maintenance Mode remains ON — no customer has seen it.** See § 13 F147 |
