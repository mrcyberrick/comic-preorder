...
| F115 | **Medium** — a never-arrived title is auto-fulfilled on schedule, so My List tells the customer "✓ Order placed" for a book that never came. **RESOLVED on staging 2026-08-28** (S1/S5/S6 all ran for real, V1/V4/V5 all green, backfill 32/30 applied). **Production ran its real Sept import same day** — write confirmed live (`arrived=212, unknown=6, not_arrived=0`) — but stays open pending its own backfill | Owner: `docs/f115-arrival-truth-persistence.md`. **Staging: DONE.** Production: import ran, write confirmed live; **S6 backfill still owed — 859 pre-existing orphaned rows measured 2026-08-28, not yet written**, no date set, Rick-gated. See § 13 |
