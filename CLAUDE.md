...
| F132 | **Medium** — a title restricted to a distributor allocation ratio (e.g. `1:10`) carries no signal at reservation time; customer only learns via the retrospective F117/F120 rejected badge. **Both distributors** — corrected same-day, Lunar carries the ratio in `variant_type` (562 rows, staging), not absent as first measured | Owner: `docs/order-restriction-alert-badge.md` (**STAGING COMPLETE 2026-08-21** — migration applied, real import confirmed live data on both distributors, a hover-stacking bug AND a mobile-tooltip gap (both found via Rick's real-data testing) root-caused and fixed same session — mobile now gets the disclosure via the existing detail modal, not a new popover. 210/210 unit + 6/6 Playwright green. Production not requested) |
