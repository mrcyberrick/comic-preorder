...
### Deferred — staging-only, awaiting production promotion
- **F101/F102 order-export correctness** — live and verified on staging,
  **not on production.** Promotion needs `docs/sql/order-submissions.sql`,
  then `docs/sql/get-ordered-codes-rpc.sql`, then a backfill regenerated
  against production's catalog, then `/promote-prod`. Until that happens
  production still has no FOC window, no order ledger, and no duplicate
  check — and it is production that holds the realized 5-copy surplus.
