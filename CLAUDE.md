...
- **Withdrawal detection (F110)** — a withdrawn title is *absent* from the
  next catalog file, not flagged in it, so detection is a set difference at
  import, not a column read. Closes F101's open customer-facing thread and
  is the cheaper half of F108. See § 13 F110.
- **Order Builder gathers by catalog month, not FOC cycle (F111)** — a
  distributor FOC spans several catalog months. Measured exposure is small
  (2 titles at filing) because the ordering cadence matches the catalog
  cadence, but the fix is to gather by `foc_date` across months, matching
  PRH's own FOC-page model. See § 13 F111.
- **Distributor-model facts unrepresented (F112)** — Lunar's
  `InitialOrderDue` and `TitleNote` unread by the import; a passed FOC is
  recoverable on Lunar but terminal on PRH, which the backorder panel does
  not distinguish. Second half is a product decision. See § 13 F112.

**All three were filed 2026-08-03 from the PRH and Lunar vendor
...
behaviour. That read corrected one wrong fix direction (F110) and one
wrong severity assumption (F111) before either reached code.**
