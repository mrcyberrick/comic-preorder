...
// ── Loading-height reservation (F141 follow-up, 2026-08-30) ──────────────
//
// THE DEFECT. `mylist.html` and `arrivals.html` both paint a one-line
// "Loading…" state inside a container that is then replaced by a full table or
// grid. The container grows in a single step and shoves the whole page down:
// measured desktop CLS 0.613 on My List and 0.966 on Arrivals, against 0.636
// for the catalog defect F141 fixed. Mobile is fine on both (0.016 / 0.076) —
// CLS is scored against viewport height, and on a phone the content already
// exceeds the screen before it fills.
//
// WHY NOT renderSkeletons(). F141 reserved the catalog grid's height with
// skeleton cards, and that was right THERE because the row count is a known
// constant (PAGE_SIZE) and every card is the same shape. Neither holds here:
// the row count is whatever this customer reserved, and the two pages have
// different layouts (a table vs a cover grid). Fixed skeletons would reserve a
// height nobody's list actually is — trading one shift for a smaller one, and
// adding two skeleton geometries to keep in sync with two real ones.
//
// WHAT THIS DOES INSTEAD. Remember how tall this container rendered last time
// for this viewer, and reserve exactly that while loading. A returning visitor
// — which on an authenticated page is nearly everyone — gets a near-zero
// shift, because their list is about the size it was yesterday. A first-time
// visitor gets `fallbackPx`, still far better than one line of text.
//
// The reservation is a floor (`min-height`) and is RELEASED as soon as real
// content lands, so a list that shrank does not leave dead space below it.
//
// Storage is per-viewer, per-key, and entirely optional: every access is
// wrapped, and a browser that blocks site data (private windows, cleared data,
// some embedded views) simply gets `fallbackPx` and behaves correctly.
const LoadingHeight = {
  ...
  reserve(el, key, fallbackPx) {
    if (!el) return;
    let px = 0;
    try { px = parseInt(localStorage.getItem(this._key(key)), 10) || 0; } catch (_) {}
  ...
  // Call AFTER real content has been written into the container. Releases the
  // floor first, then measures what the content genuinely needed.
  remember(el, key) {
    if (!el) return;
    el.style.minHeight = '';
    // rAF so the measurement happens after layout settles, not mid-write.
    requestAnimationFrame(() => {
      const h = el.offsetHeight;
      if (h > 0) { try { localStorage.setItem(this._key(key), String(h)); } catch (_) {} }
    });
  },
};
