...
| S4 | ~~**(Deferred) Provision `rjbookstop.pulllist.app`**~~ → **DONE — live, date unrecovered (F145)** | *Deprioritized 2026-07-20 — founding stays on the apex.* If/when wanted: manual CF custom domain + TLS (5.5 procedure). Not part of the driving marketing-page work. The same manual flow provisions any *premium* tenant's subdomain. **⚠ Corrected 2026-08-27 (F145): this row was stale. `rjbookstop.pulllist.app` IS provisioned and serving (curl 200, A/AAAA present), as is `comicstore.pulllist.app` — each an individual CF Pages custom hostname. There is NO wildcard: an arbitrary subdomain returns NXDOMAIN. When and why it was provisioned is recorded nowhere; recover the date from Cloudflare's audit log if it matters. This hostname now appears on printed customer material (print "View Online" CTA, PR #140) and must not be retired without reprinting.** |
