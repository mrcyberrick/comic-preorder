...
| 2026-06-11 | S4 | In progress — PR `feat/5.1-redirects-prod` → `main` open; staging 301 verified; prod promotion pending Rick's merge | `_redirects` at repo root: `/comic-preorder/* /:splat 301` and `/comic-preorder-staging/* /:splat 301`. F59 diff assertion: app files identical (infra-only promotion); `config.js` not in diff. Staging verify: `curl.exe -sI "https://staging.pulllist.pages.dev/comic-preorder/catalog.html?x=1"` → `301` `Location: /catalog.html?x=1`. Prod 301 verify pending post-merge. |

---

**Last updated:** 2026-06-11 (S3 complete; S4 PR open — prod promotion pending Rick's merge)
