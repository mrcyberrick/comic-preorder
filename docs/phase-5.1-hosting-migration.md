...
| 2026-06-11 | S3 | Complete | Local-only edits (never committed): `playwright.config.ts:21` → `baseURL: 'https://staging.pulllist.pages.dev/'`; `.env` `STAGING_REDIRECT_URL` → `https://staging.pulllist.pages.dev/catalog` (no `.html` — CF Pages pretty-URL redirect strips `.html` server-side and loses the auth hash fragment; extensionless target avoids the redirect chain entirely); `waitForURL` / `toHaveURL` patterns in `fixtures/auth.ts` (lines 87, 97), `tests/01-magic-link-arrival.spec.ts` (line 5), `tests/04-arrivals-this-week.spec.ts` (lines 94, 123, 136), `tests/07-tenant-isolation.spec.ts` (5 occurrences) widened from `/catalog\.html/` to `/\/catalog/` (and admin fixture from `/(catalog\|admin)\.html/` to `/(\/catalog\|\/admin)/`). **Playwright full suite: 15/15 passed in 1.6 minutes** (all tenant-isolation specs included). Rick manual verify pending (load `https://staging.pulllist.pages.dev/`, log in, reserve → mylist → cancel). `git push staging staging:main` retired operationally — staging deploys via `git push origin staging` only from this point forward. |

---

**Last updated:** 2026-06-11 (S3 complete — Playwright 15/15 green against CF Pages staging; Rick manual verify pending)
