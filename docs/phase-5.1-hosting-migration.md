...
| 2026-06-11 | S3 | Complete | Local-only edits (never committed): `playwright.config.ts:21` → `baseURL: 'https://staging.pulllist.pages.dev/'`; `.env` `STAGING_REDIRECT_URL` → `https://staging.pulllist.pages.dev/catalog` (no `.html` — CF Pages pretty-URL redirect strips `.html` server-side and loses the auth hash fragment; extensionless target avoids the redirect chain entirely); `waitForURL` / `toHaveURL` patterns in `fixtures/auth.ts` (lines 87, 97), `tests/01-magic-link-arrival.spec.ts` (line 5), `tests/04-arrivals-this-week.spec.ts` (lines 94, 123, 136), `tests/07-tenant-isolation.spec.ts` (5 occurrences) widened from `/catalog\.html/` to `/\/catalog/` (and admin fixture from `/(catalog\|admin)\.html/` to `/(\/catalog\|\/admin)/`). **Playwright full suite: 15/15 passed in 1.6 minutes** (all tenant-isolation specs included). Rick manual verify: **passed** (2026-06-11 — reserve → mylist → cancel confirmed). `git push staging staging:main` retired operationally — staging deploys via `git push origin staging` only from this point forward. |
| 2026-06-11 | S4 | Complete | `_redirects` at repo root: `/comic-preorder/* /:splat 301` and `/comic-preorder-staging/* /:splat 301`. F59 diff assertion: app files identical (infra-only promotion); `config.js` not in diff. Staging verify: `curl.exe -sI "https://staging.pulllist.pages.dev/comic-preorder/catalog.html?x=1"` → `301` `Location: /catalog.html?x=1`. Prod verify (post-merge): `curl.exe -sI "https://pulllist.pages.dev/comic-preorder/catalog.html"` → `301` `Location: /catalog.html`. |

---

| 2026-06-11 | S5 | Complete | **Path 1 — apex `pulllist.app`.** **DNS pre-capture (Namecheap, 2026-06-11):** CNAME `www` → `parkingpage.namecheap.com.` (30 min); URL Redirect `@` → `http://www.pulllist.app/` (Unmasked); TXT `@` → `v=spf1 include:spf.efwd.registrar-servers.com ~all` — all Namecheap parking defaults; no MailerSend, no MX, no production records (new domain, safe to proceed). CF additionally imported A `pulllist.app` → `162.255.119.129` (proxied) and CNAME `www` → `parkingpage` (proxied) — both deleted before zone activation; MX ×5 eforward*.registrar-servers.com and matching TXT (DNS only) — harmless, left in place. **Nameservers switched:** `morgan.ns.cloudflare.com` / `tia.ns.cloudflare.com` at Namecheap; zone → Active 2026-06-11. **Custom domain:** `pulllist.app` → Active on CF Pages project `pulllist`. **Verifications (Claude):** `pulllist.app/` → `200` + `CF-RAY` ✓; `config.js` → prod ref `plgegklqtdjxeglvyjte` ✓; `pulllist.app/comic-preorder/catalog.html?x=1` → `301` → `Location: /catalog.html?x=1` ✓. **Write-smoke (Rick):** reserved item; preorder row `d954fdc6-6687-42bf-8962-8819c9a1bfe7`, `tenant_id` = `20941129-c35a-476d-ae21-44b8f77af89c` (prod founding tenant ✓); reservation cancelled ✓; MailerSend email delivered ✓. **Soak clock starts: 2026-06-11 ~22:10 EDT. Soak completes: 2026-06-14.** Out-of-scope observation: Supabase cron webhook "PROD APP ONBOARDING" → `register-customer` returning 401 → filed as F68 (pre-existing, not caused by 5.1). |

**Last updated:** 2026-06-11 (S5 complete — prod live at pulllist.app; soak running through 2026-06-14; next = S6 doc rewrite)
