...
- **Status:** filed 2026-07-17 (surfaced on the first monthly prod import after F75's 2026-07-15 key rotation). **Open — fix implemented on staging branch `fix/f87-notify-customers-probe` (`19c23f9`), capability-probe approach verified at the API level on staging; pending staging function deploy + end-to-end notify test, then prod promotion (gated on Rick's OK) + re-send of the July email.** Regression from F75; related to F86 (the legacy JWT being left live is why the function's *own* service calls are unaffected).
