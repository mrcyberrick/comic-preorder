...
- **Interim mitigation, no code:** confirm the title against the distributor's site (as done here), then a scoped `UPDATE catalog SET withdrawn_at = NULL, withdrawn_last_seen_month = NULL WHERE …` for that specific row. No built-in remediation path exists today beyond a direct DB correction.
- **Proposed fix shape, not committed to:** split `detectWithdrawals()` so the clear-on-reappearance half runs on **every** import (new-month or same-month refresh), comparing currently-withdrawn rows against the freshly upserted `currentRecords` regardless of `isNewMonth`; leave the mark half exactly as new-month-gated as it is today. Clearing a false positive is always safe; only marking needs the full-month set comparison.
- **Where:** `scripts/import.js` `detectWithdrawals()` (private scripts repo) and its call site (~line 2035-2038); no `admin.html` or schema change anticipated.
