...
- **Status:** filed 2026-08-21. Discovered by Rick cross-checking the Arrivals admin "Not in shipment" count against Lunar's own site for SPAWN SCORCHED #54 (`0626IM0402`): the app showed in-store `8/26/2026`, the distributor showed `9/2/2026` (FOC pushed to `8/10/2026`). Reproduced and fixed live via three sequential backfill imports, oldest-to-newest (June→July→August), first on staging (`import-staging.js`) then on production (`import.js`). **Owner: `docs/f136-catalog-month-integrity.md` (IN PROGRESS — S1+S2 shipped 2026-08-22).** Root cause **identified 2026-08-21** — see below; fix shape sized into three sessions there.
