...
- **Status:** open — intentional prod divergence retained; staging needs audit. Re-confirmed 2026-06-10 (4.8 H4 structural diff): prod has `admins manage tenant profiles` (ALL, `TO authenticated`) on `user_profiles`; staging does not — matches F58 as originally written. **Scheduled: sub-deploy 5.0 step S3** (audit → decision gate → fix; `phase-5.0-pre-phase-5-housekeeping.md`). Planning-time evidence (2026-06-10, § 1.2 of the 5.0 plan): suspend/delete are authenticated-client PostgREST calls (`app.js:916`/`925`; `admin.html:1608` Decline), and the staging Pending-tab Decline silently no-ops — consistent with the missing-policy branch.
...
- **Status:** open — assessed 2026-06-10 (Phase 4 completion audit): **safe to fix on staging; staging-only DDL; does not block Phase 4 closure.** **Scheduled: sub-deploy 5.0 step S1** (`phase-5.0-pre-phase-5-housekeeping.md`).
...
- **Status:** open — filed 4.8 H5 candidate (2026-06-10). Same defect class as F61; deferred out of 4.8 scope per Rick. **Scope extended 2026-06-10 (Phase 5 planning):** also covers `mylist.html:1081` (the unsubscribe guard F61's resolution deferred "tracked as F65" — it was missing from this entry's Where/Fix). **Scheduled: sub-deploy 5.0 step S6** (`phase-5.0-pre-phase-5-housekeeping.md`).
...
- **Status:** open — filed 2026-06-10 (Phase 4 completion audit, during F64 item-4 assessment). Latent, currently unreachable; do not fix inline. **Scheduled: sub-deploy 5.0 step S4** (paired with F64 item 4; `phase-5.0-pre-phase-5-housekeeping.md`).
