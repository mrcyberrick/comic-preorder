...
- **Status:** filed 2026-07-25 (surfaced while investigating an unrelated Pages build failure plus Rick's report that Brevo campaigns were being suspended), **open — deferred to a future session.** The *underlying* send failure was root-caused and fixed the same session (see Root cause below); the **detection gap is what this finding tracks** and is unfixed.
...
- **Status:** filed 2026-07-25 (surfaced minutes after the F96 fix, when the first newsletter ever actually delivered arrived with visible gaps), **open — deferred to a weekly-pipeline hardening session.** The symptom was cleared same-session by a manual Pages rebuild; **the race itself is unfixed and recurs on every import.**
...
- **Fix direction (future session):** replace the per-file Contents API commits with a **single Git Data API commit** — create blobs, build one tree, one commit, one ref update — so an import produces exactly one commit and one Pages build. That removes the race rather than masking it, and also eliminates the ~30 cancelled workflow runs per import.
...
- **Related:** **F96** — same pipeline, and the two compound badly: F96 means the GitHub Action reports success unconditionally, F98 means images fail without any error, so **a broken newsletter currently produces no signal anywhere**. The only detection is a human opening the delivered email, which is exactly how both were found. Fix them in the same pass.
