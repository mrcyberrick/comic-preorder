...
- **Status:** filed 2026-08-09 while scoping the Accounts tab. **PARTLY RESOLVED 2026-08-10 — read the split carefully, the two halves are in different states:**
  ...
  - **Database gate: WRITTEN, NOT RUN.** `docs/sql/f127-account-status-write-gate.sql` — one `STABLE SECURITY DEFINER` helper plus **four RESTRICTIVE policies** (INSERT/UPDATE on `preorders` and `subscriptions`). **No DDL has been executed on either environment.** Until it is, `status` remains a **UI gate, not an authorization boundary**, and the HTTP 201 probe that filed this finding would still succeed.
