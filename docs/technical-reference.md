...
- **Status:** filed 2026-07-10, **open** — root fix deferred to the `import.js` session; a one-time prod data cleanup consolidates existing duplicates (SQL below, Rick-in-the-loop).
...
- **Root fix (deferred — `import.js`):** when the monthly import lands a new catalog row for an `item_code` that already exists in a prior month, **carry the reservation forward instead of duplicating** — i.e. **upsert the preorder onto the newest catalog row (most recent on-sale date) while retaining the original reserved date** (`created_at`), then retire the superseded row. Equivalently, auto-reserve/reserve must dedupe by `item_code` across months, not by `catalog_id`. Belongs in the `import.js` catalog-upsert session alongside F78's existing-duplicate-catalog-row reconciliation and the F75 credential refactor. No web-app change is required once the data is clean and the import stops minting duplicates.
- **One-time prod cleanup (Rick-in-the-loop):** consolidate each `(user_id, item_code)` group onto the survivor = the **newest-`catalog_month` row** (most recent on-sale date), set its `created_at` to the group's **earliest** (retain the original reserved date), take `MAX(quantity)`, and mark `fulfilled` if **either** row was; delete the superseded rows. Detection + preview + apply SQL captured in the session runbook; re-run the detection query after → 0 rows.
