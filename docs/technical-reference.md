...
- **Status:** filed 2026-07-17; **staging verification complete 2026-07-17 (F86 execution session) — predicted failure mode did NOT reproduce; prod verification pending Step 5 (V6)**. Related: F86 (same toggle; the `config.js` anon side), F87 (same function/auth surface — this would silently re-break the fix), F75 (key-rotation origin). Surfaced while implementing F87.
- **Severity:** High **if confirmed** — flipping the F86 toggle would 401 the service calls inside all ~9 prod edge functions (customer notification blast, invites, paper-customer create/claim, approve, send-my-list, reset-password, register-tenant/customer), i.e. a broad outage of the function layer. No data corruption; functions fail closed. **Downgraded to Medium pending prod confirmation** — staging evidence (below) strongly contradicts the predicted mechanism, and Step 5's rollback (re-enable legacy keys, immediate, officially supported) already covers this exact failure mode if it does occur on prod.
...
- **Scope:** prod (all edge functions) — verification above is staging-only; prod's project could in principle behave differently, so this is not yet fully closed. **Fix / next step:** proceed to F86 Step 5 (prod toggle flip) with its existing V6 gate (immediate post-toggle Edge Function exercise) as the prod-side confirmation; Step 5.4's rollback (re-enable legacy keys) is the mitigation if prod diverges from staging's behavior. Close this finding fully once V6 passes.
