...
- **Status:** filed 2026-07-25 (surfaced during the F96 Brevo investigation, from Brevo's sender-compliance panel), **open — deferred.** DNS-only change; no code touched.
...
- **Related:** F96 (same investigation). **F72** (multi-tenant email branding) — whenever per-tenant sending domains land, each needs its own DKIM/DMARC and must not inherit this gap; worth resolving F97 first so the founding domain is a correct template.
