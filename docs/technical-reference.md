...
- **Status:** filed 2026-08-02 during the planning interview for the F101/F102 session (`docs/order-export-foc-window-and-order-state.md`). **Open — deferred to its own session; not started.** Explicitly scoped OUT of the F101/F102 session by Rick the same day, on the grounds that stopping the duplicate-order bleed comes first and this needs sample invoice files before it can be planned concretely.
...
- **Status:** filed 2026-08-03 while reviewing PRH's *Direct Market Catalogs and FOC Pages* documentation and both distributors' live catalog files at Rick's request, specifically to avoid designing against assumed behaviour. **Open — deferred; needs a decision on which PRH file variant to pull.** No plan doc.
...
- **Fix direction (future session), two viable paths:**
  1. **Set difference at import.** `import.js` already computes the current month's `item_codes` array for `delete_dropped_catalog_items()`. The codes it would drop but cannot (because reservations block deletion) are precisely the withdrawal candidates — surface them instead of discarding them.
  2. **Pull a different PRH file.** PRH's documentation describes *Monthly Catalog CSV* / *Weekly Catalog Delta* downloads that do carry `PP–Postponed` and similar. Switching to (or additionally pulling) that variant would give a structured signal for PRH — but **not for Lunar**, which has none, so path 1 is still needed for full coverage.
...
- **Status:** filed 2026-08-03 from PRH's *Direct Market Catalogs and FOC Pages* documentation, verified against live production data. **Open — enhancement, not a defect in the shipped behaviour.** No plan doc.
...
- **Fix direction (future session):** gather the Order Builder's candidate set by **`foc_date` across all catalog months** rather than by `catalog_month`, which is what PRH's FOC page already does. Two things to settle first: (a) de-duplication, since a re-listed title has a row per month and the code is the identity that matters (§ 4.11); (b) whether the backorder-risk panel adopts the same widening — it should, since it currently cannot see a reservation whose FOC locks today if that reservation sits in a prior month.
...
- **Status:** filed 2026-08-03 from Lunar's ordering FAQ and its live product file, read alongside PRH's documentation. **Open — deferred; product decision needed on the second half.** No plan doc.
...
- **(b) A passed FOC is not equally fatal on both distributors.** Lunar's FAQ: *"As long as quantities are available and the items have released, items ordered after the FOC date will be sent in the next scheduled shipment… If the system notes the orders after the FOC deadline as backordered, the items may not all fill in the release week."* So on Lunar a missed FOC is **recoverable but not guaranteed**. On PRH it is terminal — an unconfirmed cart simply does not enter the order (*"Carts must be confirmed before their FOC date… Any unconfirmed quantities will not be included"*), which is how the MIDNIGHT X-MEN #2 codes came back UNKNOWN. **The § 4.5 backorder-risk panel treats both identically**, overstating severity for Lunar titles and, more importantly, obscuring that a Lunar Backordered title is still worth chasing.
...
- **Fix direction (future session):** read `InitialOrderDue` and `TitleNote` into `catalog` (two additive columns); then decide whether the backorder-risk panel distinguishes Lunar ("act now — still orderable, may not fill") from PRH ("missed — cannot be ordered"). The second is a product decision, not a mechanical one.
