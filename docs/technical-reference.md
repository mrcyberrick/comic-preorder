...
- **Rollout in progress — started 2026-07-28. Steps 4 and 5 are still owed; F9 does not close until step 5.** Deliberately sequenced so the old and new indexes coexist throughout, meaning no window exists in which either script version is broken:
  ...
  | 4. Prod `CREATE INDEX` + `import.js` | ⬜ **owed** | Same index on `plgegklqtdjxeglvyjte`, then mirror the one-line change into `import.js`. **Index first** — reversing the order breaks the next prod import with `42P10`. |
  | 5. `DROP INDEX weekly_shipment_unique` (both) | ⬜ **owed** | Only after step 4 is verified. **This is the step that actually closes F9** — until the old index is gone, the cross-tenant collision it enforces is still the live constraint. |

- ⚠️ **Nothing automated will notice if steps 4–5 are skipped.** The scripts repo's parity suite passed 46/46 on 2026-07-28 *while `import.js` and `import-staging.js` genuinely disagreed on this conflict key*, because it covers the row builders and not the write URLs. That is correct during a staging-first rollout, but it means the suite is **not** a backstop here: a half-finished rollout looks exactly like a finished one to every automated check. Recorded explicitly because this is the F105 failure shape — a real precondition whose only tracker is prose. The Step 7 checkbox in `docs/tenant-onboarding-runbook.md` remains the operative gate until step 5 lands.
...
- **Status:** filed 2026-07-27 alongside F101, from the same production reconcile. **Open — deferred; not started.** No plan doc. **The live instance is confirmed realized, not hypothetical — Rick verified the same day that PRH holds an order for 12 copies.**
