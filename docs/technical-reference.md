...
- **Status:** filed 2026-07-25 (surfaced during the arrivals shipment-reconciliation work), **open — deferred to a future test-infrastructure session.** Unrelated to that feature's diff; noticed only because the reconciliation panel reads `user_profiles` for customer names, which put the leftover rows in view.
...
- **Status:** filed 2026-07-25 (surfaced while investigating an unrelated Pages build failure plus Rick's report that Brevo campaigns were being suspended). The *underlying* send failure was root-caused and fixed the same session (see Root cause below); the **detection gap is what this finding tracks**. **Fix deployed 2026-07-26** (weekly-pull-feed commit `34074c3e`) and proven against a stubbed API, but the finding stays **OPEN** until the gap is observed closing on the real thing — a deliberately-suspended campaign turning the Action red, with GitHub emailing the failure (gate V4 in `docs/weekly-pipeline-hardening.md`). Marking it resolved on stubbed evidence alone would repeat the exact mistake it documents: trusting a green signal that was never verified end to end.
...
- **Status:** filed 2026-07-27 during a title-reconcile question about two PRH codes rejected as UNKNOWN. **Open — deferred; not started.** No plan doc.
...
- **Status:** filed 2026-07-27 alongside F101, from the same production reconcile. **Open — deferred; not started.** No plan doc. **One live instance is unresolved and needs a call to PRH before any code changes — see below.**
- **Severity:** Medium-to-High — this is a duplicate-ordering path with direct cost exposure, and it is systematic rather than specific to one title. It bites hardest on exactly the titles that carry the most advance risk: long-lead solicitations that sit unarrived across several catalog months. No data-integrity or security exposure. Both environments.
...
- **Live instance (production, verified 2026-07-27) — unresolved:** `75960621668000111` (MIDNIGHT X-MEN #1) exists as three catalog rows — `2026-05` and `2026-06` both at FOC `2026-07-06` / on sale `2026-08-05`, and `2026-07` at FOC **`2026-08-31`** / on sale **`2026-10-07`**. PRH pushed the book back nine weeks and re-listed it under the identical code. The archived June submission `Orders Archived/prh-order-2026-06-27.txt` contains **`75960621668000111,5`** — five copies already ordered. The current cycle's sheet carries **seven more** against the same code. Whether that is a true duplicate depends on whether PRH rolled or cancelled the June order when they re-dated the title; PULLLIST cannot distinguish the two and neither can the archived artifacts. **Ask PRH before submitting this cycle.**
...
- **Detection gap worth noting:** like **F96**, the failure produced no signal — the June order and the July sheet are both individually correct, and only holding the two archived files side by side reveals the overlap. Any fix should be judged on whether it would have made this visible without someone going looking.
