...
- **Status:** filed 2026-07-17 (surfaced on the first monthly prod import after F75's 2026-07-15 key rotation). **Open — fix in progress this session (staging first; prod promotion gated on Rick's OK).** Regression from F75; related to F86 (the legacy JWT being left live is why the function's *own* service calls are unaffected).
...
- **Fix (planned, this session on staging):** make the service-role detection format-agnostic — recognize the new `sb_secret_` secret-key format in addition to the legacy `service_role` JWT — instead of depending solely on the manually-decoded JWT `role` claim. Must **not** be `token === SUPABASE_SERVICE_ROLE_KEY` equality: that env still resolves to the legacy JWT (F86 — legacy keys left live) while the import now sends the new key, so they will not match. Verify the exact detection approach against Supabase's current API-key scheme before landing; verify end-to-end on staging via a real (non-`--no-write`) `import-staging.js` service-role call before any prod promotion.
- **Also noted (reconcile when landing the fix, not a separate finding):** the function's inline comments contradict each other on platform JWT verification — lines 19-20 say "JWT verification is disabled at the platform level," lines 27-28 say "platform JWT verification is ON." The security reasoning for the detection change depends on which is true.
- **Where:** `supabase/functions/notify-customers/index.ts` — the `isServiceRole` block (~lines 29-51). Related: F75 (root — key rotation), F86 (legacy keys still live).
