...
- **Still owed before this can be called done anywhere:** the **full** Playwright suite (not a targeted run) and production promotion, which is Rick's call. The full suite matters specifically here: its fixtures DELETE `preorders` as service role and **throw** on failure, so a mistake in the trigger's step-(1) exemption surfaces there and in almost no other check.
