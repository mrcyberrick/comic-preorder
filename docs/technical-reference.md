...
- **Fix shipped, 2026-08-28** (see Status above). Both `import.js` and `import-staging.js` updated identically; no schema change, no `admin.html` change — the withdrawn panel already reads `withdrawn_at`, so it reflects the clear automatically once it happens.
...
- **Status:** filed and **code fix shipped same day, 2026-08-28** (scripts repo `main` `e4f968d`). Found immediately after production's real September import (`node import.js`) — the console reported **519 withdrawn title(s) detected**, an order of magnitude past anything staging (16, see **F146**) had suggested was plausible. **Data correction to production still OWED** — see the dedicated bullet below; do not read "code fix shipped" as "the 519 are cleared."
...
- **Production data correction: NOT YET applied — this is the one piece still owed.** The 519 ids were captured to a local snapshot before any write, for exact revertibility, and `clear-f147-withdrawn.js` was written to apply the clear (none of the 519 have a passed FOC, so the script would withhold zero and clear all 519). But the CLI session's own permission classifier blocked the write when attempted directly (a bulk PATCH against production, even scoped to explicit captured ids) — correctly, by design; a production write of this shape should not go through unreviewed. The script is local-only (this repo's allowlist `.gitignore` doesn't track it) and needs to be run by Rick from the scripts folder: `node clear-f147-withdrawn.js`. **Maintenance Mode has been ON throughout and remains ON** — no customer has seen the 519 marks at any point, so there is no urgency beyond wanting the data correct before the store reopens.
