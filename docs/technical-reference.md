...
- **Status:** filed and **fixed same day, 2026-08-28** (scripts repo `main` `e4f968d`). Found immediately after production's real September import (`node import.js`) — the console reported **519 withdrawn title(s) detected**, an order of magnitude past anything staging (16, see **F146**) had suggested was plausible. Data correction to production handled via a local one-off script (`clear-f147-withdrawn.js`, not tracked — this repo's allowlist `.gitignore` keeps one-time remediation scripts local, same as other scratch files).
...
- **Production data correction:** all 519 marks cleared (none had a passed FOC, so none were withheld) — captured to a local snapshot before writing for exact revertibility, applied via `clear-f147-withdrawn.js`, verified `withdrawn_at IS NOT NULL` count back to 0 for the tenant afterward. Maintenance Mode was ON throughout; no customer ever saw the 519 marks.
