...
- **The design decision this finding exists to force:** flat `noreply@pulllist.app` versus per-tenant `<slug>.pulllist.app`. **Recommend the per-tenant subdomain.** Brevo's sender already sits at `rjbookstop.pulllist.app` — set ad hoc during the F96 incident when `SENDER_EMAIL` was repointed off the noncompliant `hello@mrcyberrick.us` — so the tenant-slug shape is already established and fully authenticated. It is also precisely what **F72** needs, since per-tenant email branding requires a per-tenant sending identity. Branding consistency and multi-tenancy then fall out of one structure. A flat apex sender would have to be torn out and redone when F72 lands.
  ***⚠️ REASSESS 2026-09-02 — this recommendation's stated rationale is now partly void.*** *It rests on "Brevo's sender already sits at `rjbookstop.pulllist.app`", and **Brevo has since been evaluated for transactional and REJECTED** (plan doc § 9), so that argument no longer supports the **transactional** sender — Brevo keeps its **marketing** sender there, untouched. Two corrections follow. **(a)** F72's per-tenant branding lives in the `from` **name** and the body copy, both secret-driven since S1 — it does **not** require a per-tenant sending **domain**, so "would have to be torn out and redone when F72 lands" overstates the cost; the subdomain buys reputation segmentation, not branding. **(b)** A flat `noreply@pulllist.app` needs exactly **one** domain slot forever at any tenant count, which decides whether a 1-domain free tier suffices — and provider free tiers differ on whether a parent domain even covers its subdomains (MailerSend yes, Resend no, SES yes). **Plan doc § 10 names this the lever to settle BEFORE choosing a provider.*
...
- **⚠️ RE-MEASURED 2026-08-30 — the population is 893, not 197, and the classification this entry asked for is now done.**
  - **893** `pw-*@example.test` auth users (of 912 total on staging), enumerated with **explicit pagination** (`/auth/v1/admin/users?page=N&per_page=200`, looped to exhaustion). The 2026-08-18 figure of 197 was taken by an unrecorded method; given GoTrue's admin list **defaults to a small page size**, an unpaginated read is a plausible explanation — the same defect class as **F82/F113/F139/F140**, which this project has now hit five times. Whether the number grew or was under-counted, **treat 893 as the measurement and 197 as unreliable.**
  - **Concentration — 5 prefixes hold 737 of 893 (83%):** `pw-prompt` **214**, `pw-iso` **207**, `pw-rsv` **169**, `pw-cust` **77**, `pw-pending` **70**. The remaining ~156 are a long tail of one-off `pw-<uuid8>` names, mostly single instances from 2026-06/07.
  - **`pw-pending` (70) must NOT be deleted wholesale** — a surviving auth row for a declined account is **intended** (F64 item 5 Option A). This is the distinction date-bucketing could never make, and it is why the recorded bulk-delete plan was invalid.
  - **Mechanism located, by counting call sites rather than guessing:** `10-post-reserve-prompt.spec.ts` (`pw-prompt`) calls `createUser` **9×** but `deleteUser` only **2×**; `11-reserved-suggestions.spec.ts` (`pw-rsv`) is likewise **9× / 2×**. Users created **inside tests** are never torn down; only the `beforeAll`/`afterAll` pair is. **Those two specs are the fix targets and account for 383 orphans.**
  - **`07-tenant-isolation.spec.ts` (`pw-iso`, 207) does NOT fit that pattern** — it is balanced at 2×/2×, so its orphans have a different cause (plausibly users created against the synthetic tenant that `globalTeardown` drops without deleting their auth rows). **Not diagnosed. Do not assume it is the same bug.**
  - **Next step, unchanged in shape but now targeted:** fix the two identified teardowns, diagnose `pw-iso` separately, then delete only what remains — **excluding** `pw-pending`. No deletion was performed 2026-08-30; this pass was read-only.
- **Measured 2026-08-18 (live staging, service-role reads) — superseded by the block above:**
...
- **Status:** filed 2026-08-20. **Variant (a) RESOLVED 2026-08-30; variant (b) RE-DISPOSITIONED — its recorded diagnosis does not match the code.** Read the 2026-08-30 block below before acting on this entry.
- **2026-08-30 — variant (a) fixed, and it was reproduced DETERMINISTICALLY first, which the original entry called intermittent.**
  - **Fix:** a `ensureDeadlineCovers(tenantId, focDate)` / `restoreDeadline()` pair in `fixtures/catalog.ts`. The describe block that depends on the At-risk classification now **owns** `order_deadline`: it reads the live value, moves it (to focDate + 1 day, the smallest sufficient change) **only if** it would break the assertion, and restores it in `afterAll` — including restoring a `null`/empty original as cleared, which a naive restore would skip. Wired into `15-order-export-ledger.spec.ts` and `06-admin-this-week-bagging.spec.ts`. This is exactly what this entry's own analysis concluded: *“only the one describe block explicitly ABOUT `order_deadline` captures and restores it — these three assume the ambient value is irrelevant.”*
  - **`focThisMonthFuture()` was deliberately NOT made deadline-aware**, and that is the fix rather than a gap. Capping the fixture FOC at the deadline fails whenever the deadline is in the **past**: no date is then both future (row stays open) *and* `<= deadline` (reads At risk), so a helper that silently returned a past date would trade a loud failure for a quietly wrong test. A comment at the helper records this.
  - **Verification, all four states measured, not reasoned:** ambient deadline + guard → **06: 3 passed, 15: 36 passed, 21: 6 passed**. Deadline forced to a **past** date (`2026-08-01`, reproducing the September condition) + guard → **3 passed**. Same past deadline with the guard **commented out** → **1 failed**, at `06-admin-this-week-bagging.spec.ts:148` `expect(riskPanel).toContainText(atRiskTitle)` — **precisely this finding's symptom.** Guard restored → **9 passed** (06 + 21 together). Staging's `order_deadline` was captured before and restored after (it is `""` — cleared by the September import; Step 7 was run on production only), re-read fresh afterwards to confirm.
  - **The recurrence date was predictable and is now moot.** With production's deadline at `2026-09-24` and `focThisMonthFuture()` = `min(today+3, month end)`, variant (a) would have fired from **2026-09-22** (today+3 = 09-25 > 09-24) and stayed red until the deadline moved. That is the kind of date this entry could not previously state.
- **⚠️ 2026-08-30 — variant (b) DOES NOT REPRODUCE, and its recorded mechanism appears wrong.** `21-arrival-resolution` was run targeted (the condition the entry says makes it fail): **6/6 passed**, both under staging's cleared deadline and under a forced past one. Reading the code rather than the entry: (i) every assertion at `21-arrival-resolution.spec.ts:131/133/144/155/156` targets a **unique stamped seeded title** (`PW … ${stamp}`), which no real catalog row can collide with, so extra real rows in the panel cannot break `toContainText`; and (ii) `#backorder-risk-panel` has **no row cap** — the only `.slice()` near its render is a copy-for-sort — so a flood cannot push a seeded row out of the DOM. **The stated failure mode (“assumes the panel holds only its fixture”) is therefore not what those assertions do.** Either the 2026-08-24 failure had a different cause, or the specs have changed since. **Do not “fix” variant (b) against the current diagnosis** — it needs a real captured failure first. Its knock-on claim, that *targeted runs are untrustworthy*, is **not currently demonstrable** and should not be relied on as a reason to distrust a targeted run.
- **Original status (retained):** filed 2026-08-20, discovered out-of-scope while running the F132 smoke gate. Test-infra only, no plan doc, no fix shape sized at filing. Not caused by F132's code (`buildComicCard()`/`catalog.html` only) — confirmed by isolating and rerunning all three affected tests independently of the F132 branch's changes.
...
- **✅ 2026-08-30, SECOND PASS — all six shared-nav pages now measured and addressed. THE ROOT CAUSE IS THREE DIFFERENT DEFECTS, NOT ONE.** This is the part worth carrying forward: the same symptom (a large desktop CLS on an authenticated page) had three distinct causes, and the fix for one made another *worse* when applied blindly.

  | Pattern | Cause | Pages | Fix | Result (Rick's own account) |
  |---|---|---|---|---|
  | **A** | A loading placeholder replaced by much taller content | my list, arrivals, subscriptions | `.loading-reserve{min-height:100vh}` on the **placeholder**, in **static CSS** | mylist 0.53→0.05, arrivals 0.81→0.09 — **LIVE, PR #145** |
  | **B** | A section starting `display:none`, **revealed above** other content, pushing it down | subscriptions, admin | bound the revealed block's height | subscriptions 0.80→0.32, admin 0.90→**0.17** |
  | **C** | Many panels filling independently, no single growing container | analytics | none attempted | 0.49, untouched |

- **Pattern A — why it must be CSS.** **CLS sums an impact fraction PER SHIFT**, so a reservation applied from JS *after* first paint splits one shift into two and can raise the total. Measured: 620px scored **0.347**, a full viewport **0.578** — reserving *more* was *worse*. Reverted (`979eb7d`).
- **Pattern B — “render them together” was already true and could not help.** `renderSuggestions()`/`renderSubs()` and admin's four render calls are already adjacent and synchronous. The shift is between the pre-render and post-render paints, not between two paints of the reveal. **Read the call sequence before proposing this fix again.**
- **Pattern B, subscriptions — fixed by CAPPING, not reserving.** The block rendered one row per reserved series: **1,760px** for a 25-series customer. Capped to the **top 5** (ranked by issues reserved), it is ~451px. Reserving its slot was then attempted and **made Rick's real number worse (0.33 → 0.55)** — reverted. Best unproven theory: `.loading-reserve`'s 100vh floor **over-reserves** on a page whose content is naturally short (2 subscriptions ≈ 200px against a ~940px floor), so releasing it shrinks the page and pulls the footer into view. **If revisited, remove that floor from this page first; do not add more reservation.**
- **Pattern B, admin — fixed by bounding, in two steps, and the suite caught the first one.** Collapsing the *whole* Order Follow-Up panel took it to 93px and CLS 0.283, but turned **3 specs red** (`21-arrival-resolution` V4, `22-f143-f144` V4/V5/V6) on `toBeVisible()`. Those specs drive the **resolve controls** — Damaged / Didn't arrive / Rejected by supplier — from inside the panel: the failure was a real workflow cost on daily arrival triage, not a stale test. Final shape: **Never Arrived stays open** (it carries the controls) inside a **capped scroll region** (`.ofu-never-scroll`, max-height 300px); **Backordered + At Risk collapse** behind a summary naming what is inside. **Specs pass UNMODIFIED (10/10)** — chosen deliberately over hiding rows, because these sort by on-sale date and staging carries ~32, so a seeded title would land outside the visible N unpredictably and the specs would go **flaky rather than cleanly red**.
- **The remaining admin height is a DATA backlog, not a UI defect.** The panel is large because the F115 S6 backfill put **23 titles on production / 32 on staging** into Never Arrived awaiting a Received/Didn't-arrive/Damaged decision. Rick has ~17 left. **Triaging them shrinks the panel at source and should take admin under 0.1 with no further code.**
- **Measurement discipline this cost two false starts to learn:**
  - **Measure as a REPRESENTATIVE user.** A synthetic probe diverged from Rick's real account **three times**: 0.014 vs 0.53 on subscriptions (probe had no subscriptions, so a 0px container moving cost nothing), 0.003 vs 0.53, and 0.048 vs 0.55. `lighthouse-auth.mjs` now takes `--seed=N` (reservations), `--subs=N` (subscriptions) and `--admin`.
  - **Lighthouse launches a COLD profile every run**, so anything remembered per-viewer is structurally unmeasurable by it.
  - **An unthrottled Playwright load reads CLS 0.000** — throttling is what surfaces the shift at all.
  - **The probe is noisy on admin** (0.897, 0.90, 1.169, 0.283, 0.606 across one session). Treat it as a change detector; Rick's DevTools reading is the acceptance test.
- **✅ LIVE ON PRODUCTION 2026-08-30 (PR #145, `f21cc56`) for My List and Arrivals.** Accepted on Rick's own signed-in account — **My List 0.53 → 0.05, Arrivals 0.81 → 0.09**, Catalog unchanged at 0.02 as the control — then re-verified against the bytes `pulllist.app` serves (`style.css` rule ×1; `mylist` ×4, `arrivals` ×2, `subscriptions` ×2; negative assertions `admin` ×0, `analytics` ×0, `app.js` `LoadingHeight` ×0). Write-smoke skipped by design — CSS and two placeholders, nowhere near the reserve path. **`subscriptions.html` shipped in the same PR and is only PARTLY fixed (0.80 → 0.53): it has a second cause, see below.**
- **RESOLVED 2026-08-30 for the three customer-facing pages — and the residual was APP-WIDE, not a My List / Arrivals defect.** Rick measured all six shared-nav pages in DevTools **on his own signed-in account**: **catalog 0.02** (the page F141 already fixed) against **my list 0.53, arrivals 0.81, subscriptions 0.80, admin 0.90, analytics 0.49**. **Catalog is the positive control** — same browser, same session, same tool, one page good and five poor, which rules out instrument bias. This entry's original “plausible on mylist/arrivals, unmeasured” understated the scope by three pages.
- **The fix:** `.loading-reserve { min-height: 100vh; }` in `style.css`, applied to each page's **loading placeholder** in the **static HTML**.
  - **It must exist at FIRST PAINT, so CSS not JS. CLS sums an impact fraction PER SHIFT**, so a reservation applied after first paint turns one shift (small → full) into **two** (small → reserved, reserved → full), each scored separately, and the total can go **up**. Measured: a JS version scored **0.347** reserving 620px and **0.578** reserving a viewport — reserving *more* was *worse*, which is backwards until you count shifts rather than pixels. Reverted as `979eb7d`.
  - **On the PLACEHOLDER, not the container**, so it vanishes with the placeholder and nothing must remember to release a floor. The reverted attempt failed here too: `mylist.html`'s empty-state branch `return`s early and never reached its release call, leaving an empty list with a stuck viewport-tall blank box.
  - **A viewport is the right unit** because CLS counts only shifts the user can **see**: if the loading state fills the screen, everything below starts off-screen and its movement is not scored.
  - **`mylist.html`'s empty state KEEPS the class.** Letting it collapse gave a shrink shift, then `loadFirstIssues()` landed a discovery grid asynchronously and grew it again — **0.613 → 1.048**, worse than no fix. Caught before promotion by measuring the empty-account case explicitly.
- **Results (seeded probe, desktop, staging):**

  | Page | Rick, real account | Probe before | Probe after |
  |---|---|---|---|
  | `mylist.html` (real list) | 0.53 | 0.969 · score 75 | **0.038 · score 99** |
  | `mylist.html` (empty account) | — | 0.613 | **0.013 · score 100** |
  | `arrivals.html` | 0.81 | 0.966 · score 75 | **0.003 · score 99** |
  | `subscriptions.html` | 0.80 | — | **0.003 · score 100** |
  | mobile (mylist / arrivals) | — | 0.016 / 0.076 | 0.035 / 0.002 |

  **Gate: 143 Playwright passed, 0 failures, exit 0**, plus 9 targeted subscriptions tests after that change. **The probe over-reads** (saturates near 0.97 where Rick reads 0.53/0.81), so it is a **change detector**; Rick's DevTools run is the acceptance test.
- **`admin.html` and `analytics.html` are NOT fixed — the pattern was applied, measured, and REVERTED** (both byte-identical to the pre-change baseline). Recorded so nobody re-applies it:
  - **`admin.html` (0.90 → 0.897).** All three shifts are `#tab-by-customer` being **pushed down**, not a container growing. The cause is above it: panels starting `display:none` — `#backorder-risk-panel`, `#withdrawn-panel` — being **revealed** once data loads. A placeholder reservation cannot help, and reserving for a panel that may never appear leaves a permanent gap.
  - **`analytics.html` (0.49 → 0.833 on the probe).** `div.container` still grows to 1933px; seven loading rows at 140px reserved only ~980px. It has **no single growing container** — seven panels fill independently — so it needs per-panel reservations sized to real content.
  - Both are **staff-only**. The three customer-facing pages are done.
- **Tooling (local-only):** `lighthouse-auth.mjs` gained `--seed=N` and `--admin`, and its path guard now **throws** rather than silently scoring the wrong page. `cls-trace.mjs` is new. **Two mechanical facts:** Lighthouse launches a **cold profile** every run, so anything remembered per-viewer is structurally unmeasurable by it; an **unthrottled Playwright load reads CLS 0.000** — throttling is what surfaces the shift.
- **⚠️ 2026-08-30, SAME DAY — THOSE MY LIST NUMBERS ARE NOT A REAL CUSTOMER'S PAGE. A fix was built against them, then reverted.** Read this before acting on the table above.
  - **What the probe was actually scoring.** `lighthouse-auth.mjs` creates a **brand-new** staging user, so it has an **empty pull list**. Confirmed from the Lighthouse JSON, not inferred: `first-issues` is **PRESENT** and `list-table` is **ABSENT**. The scored page is the empty-list *discovery grid* that `loadFirstIssues()` injects asynchronously — not the pull-list table a customer with reservations loads. **So 0.613 does not describe a real customer's My List**, and neither do the two attempted-fix numbers below.
  - **This is the 2026-08-24 mistake one step further in.** That one scored the marketing page reached via a discarded catalog load; the lesson recorded was “measure the authenticated page.” This *was* authenticated — and still wrong, because a **synthetic user's page composition is not a real user's**. The rule needs widening: *measure the authenticated page **as a representative user**.* An empty account is its own special case in this app, with its own discovery UI.
  - **`arrivals.html` is less affected by this** and its 0.966 is probably closer to real: the store-shipment grid renders regardless of reservations (the probe saw “67 titles in store shipment”). It should still be re-confirmed against a seeded user before any work.
  - **The attempted fix, and why it was reverted** (commits `5470a25`, `d3f3014`, reverted by `979eb7d`; `app.js`/`mylist.html`/`arrivals.html` confirmed byte-identical to the pre-change state):
    - A `LoadingHeight` helper reserved the container's last-known height per viewer in `localStorage`, releasing it when content landed. **620px fallback → 0.347. Viewport floor → 0.578.** Reserving *more* scored *worse*, which is backwards under “bigger reservation, less growth” — and that contradiction is what stopped a third guess and prompted instrumenting the page.
    - **It also introduced a regression:** `mylist.html`'s empty-state branch `return`s (line ~1015), so the release call at the end of `renderList()` was never reached for an empty list — the floor stayed and an empty list would render a viewport-tall blank box. The code comment claiming it “covers every exit” was false.
  - **Two mechanical facts worth keeping for whoever picks this up:**
    - **Lighthouse launches a COLD browser profile every run**, so `localStorage` is always empty. Any design that remembers something per-viewer is **structurally unmeasurable** by that harness — it will only ever exercise the first-visit path.
    - **An unthrottled Playwright load records CLS 0.000** on the same page, first visit and repeat. Lighthouse's simulated CPU/network throttling is what surfaces the shift at all. **Measure under throttling or you will conclude there is no problem.**
  - **Tooling left in place** (local-only, never committed): `playwright/cls-trace.mjs` — installs a `PerformanceObserver` via `addInitScript` before any page script runs, reports every shift with timing and source geometry, and measures a **repeat** visit in the same browser context. Also fixed in `lighthouse-auth.mjs` the same day: it waited for the requested `--path` to appear in the URL, but a magic link lands on `/catalog`, so **every path except the default timed out** — which is why this residual sat unmeasured behind a tool that looked available.
  - **Before any further work here:** seed the probe user with a realistic set of reservations so the page renders `list-table`, re-measure both pages under throttling, and only then decide whether a fix is warranted and what it should target. **The open question is no longer “how do we fix this” but “is there a problem on a real customer's page at all.”**
...
- **Related:** **F131** (the same class — a structural single-operator/single-account dependency that works perfectly until tenant count moves, and that no test can reach). **F99** (sender-identity consolidation onto `pulllist.app` — same MailerSend account, and any domain work should be aware of these quotas). ***2026-09-02: "same MailerSend account" no longer holds automatically — F99's direction is now **Resend**. Its free tier is 3,000/month but still **100/day**, so a provider move raises the monthly ceiling and does NOT dissolve this finding. Re-derive the bulk-endpoint fix direction against whichever provider is actually chosen.*** **F72** (per-tenant email branding — the other half of what makes transactional mail multi-tenant-ready). **F135** (the other side effect welded into the import script's run). **F105** (the "verification that doesn't announce failure" pattern this echoes).
...
- **Related:** F136 (the Revision Sweep this should have been part of), F122 (newest-listing
  ...
  runbook sentence).
- **FOLLOW-ON SHIPPED 2026-09-05 — staging, then PRODUCTION the same day (PR #149, merge `1212f04`). No new finding ID.** The "no surface at all" half of
