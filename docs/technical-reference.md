...
- **Status:** filed 2026-08-21. Discovered by Rick cross-checking the Arrivals admin "Not in shipment" count against Lunar's own site for SPAWN SCORCHED #54 (`0626IM0402`): the app showed in-store `8/26/2026`, the distributor showed `9/2/2026` (FOC pushed to `8/10/2026`). Reproduced and fixed live via three sequential backfill imports, oldest-to-newest (June→July→August), first on staging (`import-staging.js`) then on production (`import.js`). **Open — no plan doc, root cause of the mass duplication not yet identified, no fix shape sized.**
...
- **Root cause:** not fully identified. The dominant `{2026-05, 2026-06}` pairing across 2,621 items is consistent with one systemic event (e.g., a past import session where the confirm-month prompt was answered incorrectly for most or all of a file) rather than per-item drift, but this has not been confirmed against import history.
- **Fix shape:** not sized. Two separable problems: (a) no detection exists for a stale date on an *unreserved* catalog row — the existing report is reservation-scoped only; (b) 2,666 duplicate rows need a safe repoint-and-retire cleanup. For Lunar items, the item_code's own `MMYY` prefix is a reliable ground truth for which duplicate is canonical; PRH's UPC-style item codes (e.g. `82771403150804031`) carry no equivalent signal, so (b) needs a different resolution method there before any cleanup can run against PRH rows. Any repoint of `preorders.catalog_id` plus retirement of the orphan row needs a written, reviewed runbook given the scale — not ad hoc SQL Editor work.
- **Where:** `catalogs/scripts/import.js` / `import-staging.js` — `classifyReservedDateDrift()` / `reportReservedInStoreDateChanges()` (F122), the catalog upsert conflict key (`on_conflict=tenant_id,item_code,distributor,catalog_month`); `catalog` table (2,666 duplicate rows, production — scope on staging not yet measured).
