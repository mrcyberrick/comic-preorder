...
- **Status:** filed 2026-08-10 (surfaced while reading `subscriptions.html` end-to-end to verify F127's unverified claim that the page carries no account-status check — it does not, and this was found in the same read). **Open — not fixed.** No plan doc.
...
- **Fix direction — two independent halves, and the cheap one is not the complete one.**
  1. **Client (cheap, matches shipped precedent):** disable `.unsub-btn` during impersonation with a `title`, exactly as the subscribe buttons already are. Renders disabled, not hidden, per the existing convention.
  2. **Database (the real fix, and a product question first):** decide whether an admin *should* be able to unsubscribe on a customer's behalf. If yes, `subscriptions` needs an admin write policy mirroring `admins manage tenant preorders`, and the button should be **enabled**. If no, half 1 is sufficient and the missing policy is correct. **Do not add the policy without deciding** — it widens admin write authority on a table that currently has none, and § 7.1's note that *"admins use impersonation to manage on behalf of users"* is already wrong for this table, which is probably how the gap opened.
