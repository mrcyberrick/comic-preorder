...
- **Status:** filed 2026-08-03 while reviewing PRH's *Direct Market Catalogs and FOC Pages* documentation and both distributors' live catalog files at Rick's request, specifically to avoid designing against assumed behaviour. **Open — PLANNED, not started.** Plan: `docs/order-export-followthrough-f110-f111-f112.md` (written 2026-08-03) — detection in **Session A** (§ 4.2), surfacing in **Session B** (§ 4.5). The PRH file-variant question is decided (set difference only — see the fix direction below), and the customer-facing half is settled.
...
- **Status:** filed 2026-08-03 from PRH's *Direct Market Catalogs and FOC Pages* documentation, verified against live production data. **Open — enhancement, not a defect in the shipped behaviour. PLANNED, not started:** `docs/order-export-followthrough-f110-f111-f112.md` § 4.4, **Session B**. **Blocked on F113** — the widening needs prior-month rows, which are exactly the ones the truncated fetch discards, so F113 is that session's first step.
...
- **Status:** filed 2026-08-03 from Lunar's ordering FAQ and its live product file, read alongside PRH's documentation. **Half open, half withdrawn (2026-08-03).** (a) `InitialOrderDue` / `TitleNote` remain unread — **planned as Session A** of `docs/order-export-followthrough-f110-f111-f112.md` (§ 4.1). (b) the Lunar-vs-PRH severity split is **overruled and withdrawn** — see below; the product decision it asked for was put to Rick the same day and answered "do not distinguish."
...
- **Status:** filed 2026-08-03 during the planning session for `docs/order-export-followthrough-f110-f111-f112.md`, while checking whether F111's cross-month widening was implementable at all. **Open — scheduled as Session B's first step in that plan** (Rick's call, 2026-08-03).
