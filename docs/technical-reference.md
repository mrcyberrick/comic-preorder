...
- **Status:** filed 2026-06-19, **partially remediated**. The full write-up and remediation plan are intentionally kept **out of this public repo** to avoid signaling an exploitable condition; they live in a local-only operator note (outside the repo tree). A sanitized, past-tense entry will replace this placeholder once remediation fully lands.
- **Partial fix landed 2026-07-08** (scripts repo commit `c2e37c6`, "bring import scripts under version control, credentials extracted to .env") — re-verified against live source 2026-07-15: both `import.js` and `import-staging.js` now load credentials via `loadDotEnv()`/`requireEnv()` from a gitignored `.env`, hard-fail on a missing var, and assert their `SUPABASE_URL` targets the correct project. No hardcoded literal remains in either script. **This addresses the code-level hardcoding risk but is not full remediation** — the local note's remaining steps (rotate the staging key; migrate the prod legacy full-access JWT to a new current-generation key) have not been confirmed done and are carried into the `import.js` maintenance session alongside F78/F85.
- **Disposition:** remaining remediation (key rotation) is a dedicated follow-on session. New findings are numbered from **F76**.
...
- **Status:** filed 2026-06-25, **open — root cause fixed 2026-07-09 by F84; historical reconciliation still outstanding.**
...
- **What's actually still open:** the **duplicate `catalog` rows created before the F84 fix landed** (2026-07-09) are historical data that F84 does not retroactively clean up. These need a one-time reconciliation pass: identify duplicate `(tenant_id, item_code, catalog_month)` groups with differing `distributor`, consolidate any `preorders`/`reservation_history`/`usage_events` rows pointing at the loser onto a chosen survivor (same consolidation pattern as F85's one-time prod cleanup), then delete the loser `catalog` rows.
- **Where:** local-only `import.js` / `import-staging.js` (catalog upsert key, `buildCatalogIdMap`; not in any repo — see `CLAUDE.md` § What's tracked vs local-only) for the now-superseded hardening idea; live prod/staging `catalog`/`preorders` data for the required historical reconciliation.
- **Disposition / home:** historical reconciliation is bundled into the **`import.js` maintenance session** alongside F75 (key rotation) and F85 (cross-month carry-forward root fix) — see `docs/import-js-maintenance-f75-f78-f85.md`.
...
- **Status:** filed 2026-07-10; **prod duplicates cleaned 2026-07-10** (consolidation applied — detection query re-run returns 0 pairs; bagging list shows each comic once). **Root fix still open — deferred to the `import.js` session** (without it, next month's re-listed items will duplicate again).
...
- **Root fix (deferred — `import.js`):** when the monthly import lands a new catalog row for an `item_code` that already exists in a prior month, **carry the reservation forward instead of duplicating** — i.e. **upsert the preorder onto the newest catalog row (most recent on-sale date) while retaining the original reserved date** (`created_at`), then retire the superseded row. Equivalently, `autoReserveSubscriptions()`'s existing-reservation check (`import.js`/`import-staging.js`, keyed today on `${user_id}||${catalog_id}` against only the current month's catalog ids) must dedupe by `item_code` across months, not by `catalog_id`. Belongs in the `import.js` maintenance session alongside F78's existing-duplicate-catalog-row reconciliation and the F75 credential rotation — see `docs/import-js-maintenance-f75-f78-f85.md`. No web-app change is required once the data is clean and the import stops minting duplicates.
...
- **Where:** mechanism originates in scripts repo `import.js` / `import-staging.js` (monthly catalog upsert + auto-reserve); manifests in `admin.html` This Week bagging list (`renderThisWeek`, which correctly renders one row per preorder). No web-app defect.
