...
- **Status:** filed 2026-07-23 (native-customer-signup workstream, S2), **open — deferred, fix later (Rick's call).**
...
- **Fix direction (deferred):** investigate why `C:\Users\richa\supabase` exists and either delete it, re-link it to nothing, or otherwise ensure it can't be an accidental deploy target. Until fixed, every Supabase CLI function command against this repo **must** pass an explicit `--workdir` pointing at the repo root — the native-customer-signup workstream adopted this mitigation for its remaining S2–S5 steps.
...
- **Status:** filed 2026-07-28 (noticed while reading `.claude/hooks/guard-git.ps1` to assess whether it was safe to publish in the public repo — see the tracking decision in `.gitignore`). **Open — deferred; low priority.** No plan doc.
...
- **Status:** filed 2026-08-04 from a live triage question about a specific panel row ("Sonic the Hedgehog #88 is 36 days overdue — what does the customer see?"). **Mitigated 2026-08-04 — live on staging (client) and committed to the scripts repo (import). Production promotion is Rick's call, not yet requested.** Not fully *resolved*: the detection is now reported, but nothing yet records or tracks the outcome — that remains **F108**'s job.
...
- **Status:** filed and **RESOLVED 2026-08-04, same session** — live on staging, production promotion is Rick's call, not yet requested.
...
- **Status:** filed 2026-08-07, from the production monthly import Rick ran that evening. **Data damage repaired the same hour** (3 reservations un-fulfilled, verified before and after). **Open — the mechanism is unfixed and will recur on the next import that crosses a re-dated title's stale on-sale date.** No plan doc.
...
- **Fix direction (future session), two candidates, not yet chosen:**
  1. **Judge on the newest row, not the joined row** — `auto_fulfill_past_on_sale()` compares against the latest `on_sale_date` for that `(distributor, item_code)` rather than the reservation's own catalog row. Narrow, and fixes the symptom wherever the reservation happens to point.
  2. **Carry manual reservations forward on re-listing** — extend F85's carry-forward beyond subscriber auto-reserves. Broader, and would also fix the duplicate-state problem Jay Underhill demonstrates, but touches the import's reservation handling and needs care against F85's own survivor rules.
  Option 1 is the smaller change; option 2 addresses more of the underlying split. Worth deciding together with **F121**'s process-mapping session, since both are consequences of one physical comic being represented by several catalog rows.
- **Where:** `docs/sql/auto_fulfill_past_on_sale.sql` (the `c.on_sale_date < CURRENT_DATE` predicate); `import.js` / `import-staging.js` Step 9 and `findUnverifiedFulfillments()`; `mylist.html` past-on-sale auto-hide.
