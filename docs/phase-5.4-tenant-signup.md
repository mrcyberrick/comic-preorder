...
- [x] **S0:** prod `preorders_user_id_fkey` references `public.user_profiles(id)` with `confdeltype='a'` (NO ACTION) — `auth.users` target + CASCADE removed; pre-flight confirmed 0 blocking rows; **F64 item 5 resolved**; parent Deferred-DDL Register row closed. (Gate — landed before any S1+ work.)
- [x] **S1:** founding tenant's webhook secret lives in `tenants.settings->>'mailerlite_webhook_secret'` on staging; a service-role lookup by that secret returns exactly the founding tenant (one row).
- [x] **S2 (F34 residual):** `register-customer` resolves `tenant_id` from the per-tenant webhook secret (DB lookup) — `Select-String "tenant_id: FOUNDING_TENANT_ID"` → 0; founding webhook still routes a new customer to the founding tenant on staging; empty/bad secret → 401; **F34 resolved**, **F72 filed** (email branding deferral); email template unchanged.
- [x] **S3:** `register-tenant` deployed on staging behind `TENANT_PROVISION_SECRET`; unauthenticated call → 401; reserved/malformed slug → 400; creates tenant + first admin + seeded `settings`/`branding` via service-role; **EF inventory → 9**.
- [x] **S4:** a canary tenant created **through** `register-tenant` on staging; `register-customer` with the canary secret routes a customer to the **canary** tenant (un-pin proof); full Playwright incl. tenant-isolation (F15/F20) green with two tenants present; FK-ordered teardown executed; **`canary_tenant_rows = 0` and founding intact** verified by SELECT.
- [x] **S5:** negative-path matrix green (operator-gate 401; reserved-slug 400; malformed-slug 400; partial-failure compensation leaves no orphan tenant — exercised in S4; duplicate-slug 409 code-reviewed/grep-verified, not exercised end-to-end); founding register flow unchanged; full Playwright green × 2.
- [x] **S6:** prod founding webhook secret migrated to `tenants.settings` and verified (one row, prod founding UUID) **before** the un-pinned deploy; `TENANT_PROVISION_SECRET` set on prod; both EFs deployed (JWT-verify OFF); prod founding webhook still routes to founding; `register-tenant` rejects without the operator secret; prod canary skipped (honest skip — S4 canary already proven teardown end-to-end on staging); founding write-smoke passed.
- [x] **Founding-tenant behavior unchanged** (parent invariant): full Playwright incl. tenant-isolation green at the S2/S3/S4/S5 staging gates and the S6 prod write-smoke; no `app.js`/`*.html`/`config.js` change; founding customer surface byte-identical to 5.3.
- [x] **Teardown proven before the flow opens** (parent rollback row 5.4): the § 4.1 FK-ordered teardown exercised in S4 with a verifying 0-row SELECT.
- [x] § 13 updated (**F34 → resolved**, **F64 item 5 → resolved**, **F72 filed**, **F73 resolved**, **F74 filed+resolved**, `register-customer` + `register-tenant` contracts, EF inventory → 9); Deploy Log complete (one row per executed step); all doc changes committed to `staging`; parent row 5.4 → **Complete** + date; `CLAUDE.md` pointer advanced to 5.5 planning.
...
| 2026-06-16 | S3 | Green | `register-tenant` created (operator-secret gate, slug format + reserved denylist, service-role tenant insert with seeded `settings`/`branding`, GoTrue admin user, `user_profiles` insert `status='active'`, best-effort compensation). `Select-String` verified gate/denylist/`23505`/GoTrue-create all present. Deployed to staging manually via dashboard paste (JWT-verify OFF confirmed). **Deploy friction:** initial probes returned 401 even with a correct-looking secret; root-caused via a temporary length-only diagnostic log (`configured_len=undefined`) to a **typo'd secret name** in the dashboard (`TENANT_PRVISION_SECRET` vs `TENANT_PROVISION_SECRET`) — not a code defect. Corrected, diagnostic line removed, redeployed clean, re-verified. Probes on the clean deploy: no secret → 401; malformed slug (`"Bad Slug!"`) → 400 `invalid_slug`; reserved slug (`admin`) → 400 `slug_reserved`. No real tenant created. A stray local test-payload JSON file was created inside the repo working tree during testing and deleted before commit (not staged/committed). § 13 EF inventory → 9 (§ 11.1–11.3 updated: `register-tenant` contract note added; `register-customer` row/notes corrected to reflect the S2 per-tenant-secret resolution, which had not been propagated into § 11 during S2). Feature branch → staging (ff-only) pending. |
| 2026-06-16 | S4 | Green | Canary tenant `canary54` created end-to-end through `register-tenant` (tenant_id `fd07efe4-…`, admin `9c69fcca-…`, seeded `mailerlite_webhook_secret`). Tagging verified (tenant row + admin profile). `register-customer` called with canary webhook secret → 200, new pending customer (`4a9d5345-…`) landed in `tenant_id = fd07efe4-…` (canary), not founding — **un-pin headline proof**. 19/19 Playwright green including F15/F20 tenant-isolation with two tenants present; founding behavior unchanged. FK-ordered teardown (§4.1 delete order) executed scoped to `fd07efe4-…`; post-teardown group-by shows founding only (103 profiles); `canary_tenant_rows = 0`, `canary_profiles = 0`. Teardown proven before flow opens. **Note:** initial retry 409 `admin_email_exists` diagnosed as incomplete Phase 4.7 teardown (stale `canary-admin@example.invalid` in auth.users); deleted before retry. Compensation correctly cleaned the partial tenant row on the 409 attempt. |
| 2026-06-16 | S5 | Green | register-tenant negative matrix: no-secret → 401; malformed slug → 400 `invalid_slug` (S3); reserved `admin` → 400 `slug_reserved` (S3); reserved `www` → 400 `slug_reserved`; reserved `raysandjudys` → 400 `slug_reserved`. register-customer empty/bad secret → 401. Compensation (partial-failure / no orphan row) exercised incidentally in S4 (admin_email_exists → compensation cleaned canary54 tenant row before retry). Founding register-customer routing confirmed in S2. `slug_taken` (409 / `23505` mapping) code-reviewed + grep-verified present; not exercised end-to-end (no non-reserved slug in DB to collide against post-canary teardown — noted as an honest gap). Playwright 19/19 green × 2 (S4 + independent S5 run). **F73 rotation flagged as overdue** (disposition: rotate after S2 verification; S2 + S4 teardown now complete — must rotate staging founding webhook secret before S6). |
| 2026-06-17 | S6 | Green | Prod founding webhook secret migrated to `tenants.settings->>'mailerlite_webhook_secret'` for `20941129-…` (rjbookstop); verified one row returned. `TENANT_PROVISION_SECRET` set on prod. Both `register-customer` (un-pinned) and `register-tenant` (new) deployed to prod (JWT-verify OFF confirmed). Prod gate probes: `register-tenant` no secret → 401; `register-customer` with prod founding secret → 200; `user_profiles` row verified `tenant_id = 20941129-…`, `status = pending`; test user deleted (user_profiles + auth.users). Founding write-smoke: reserved item on pulllist.app, confirmed `tenant_id = 20941129-…` in prod preorders, cancelled. F73 resolved (staging secret rotated 2026-06-17). F74 filed (prod founding secret pasted in chat) → resolved same session (rotated after verification). |
| | S7 | | |

---

**Last updated:** 2026-06-17 (5.4 S6 — prod promotion verified; founding write-smoke clean; F73+F74 resolved)
