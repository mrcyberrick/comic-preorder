...
- [x] **S0:** 5.4 artifacts confirmed live on prod (`register-tenant` gated 401-without-secret; `register-customer` un-pinned; prod founding webhook secret in `tenants.settings`); operator tenant-2 inputs validated (slug passes format + not reserved); §4.1 teardown matches live schema; prod `import.js` confirmed founding-pinned; Cloudflare access confirmed. (2026-06-18)
- [x] **S1:** staging full-dress rehearsal — tenant created **through** `register-tenant`; branding renders; `?t=<slug>` + host-parse resolution correct; full 4.1-style isolation matrix = 0 both directions with two tenants present; full Playwright (F15/F20) green; FK-ordered teardown executed; **`rehearsal_tenant_rows = 0` and founding intact** verified by SELECT. (2026-06-18)
- [x] **S2:** real tenant 2 live on prod via `register-tenant` (tenant_id/admin/webhook_secret captured locally, **no secret in chat**); branding set; `<slug>.pulllist.app` custom domain **Active** with TLS issued; subdomain routes to tenant 2 and `pulllist.app` still routes to founding; tenant-2 admin signs in to an empty tenant-2-scoped admin surface; MailerLite webhook configured or deferred-with-note; founding write-smoke clean. (2026-06-19)
- [x] **S3:** prod cross-tenant isolation matrix = **0 in both directions** across every tenant-scoped table; analytics/admin/customer-page/Edge-Function paths each tenant-scoped; `register-customer` with tenant-2 secret lands in tenant 2 (founding secret still → founding); branding correct per host; full Playwright green. (Parent headline gate.) (2026-06-19)
- [x] **S4:** two-tenant prod soak passed — **at least one full monthly import cycle** elapsed with both tenants present; founding import touched only founding rows and tenant-2 seeded data was unchanged; **post-import** isolation re-verification = 0 both directions; founding Playwright green and no customer-reported regressions across the span; tenant 2 pilot/seeded throughout; soak log complete. (2026-07-15; import cycle 2026-07-08→10; soak log § S4 close)
- [x] **S5:** `docs/tenant-onboarding-runbook.md` written — repeatable tenant-N+1 onboarding incl. the F72 email-branding check and the one-way-after-real-writes rollback note. (2026-06-20)
- [x] **Founding-tenant behavior unchanged** (parent invariant): no source file changed; full Playwright incl. tenant-isolation green at the S1/S3 gates and the S4 post-import write-smoke; `pulllist.app` byte-identical to 5.4.
- [x] **Teardown clean throughout the soak** (rollback tier): tenant 2 pilot/seeded; the §4.1 FK-ordered teardown remains a clean rollback for the entire 5.5 window (real-customer go-live is post-close).
...
| 2026-06-18 | S1 | Green | Rehearsal tenant `rehearsal55` (`699328d9-…`) created end-to-end through `register-tenant` (staging) → 200 with all four fields. Branding `{"primary_color":"#d62839"}` confirmed in DB; incognito browser verified red accent + "Rehearsal Comics (5.5)" display name; founding branding covered by Playwright test 17 (no-op). `?t=rehearsal55` routing resolves to rehearsal tenant ✓; host-parse unit check: `tenantSlugFromHostname('rehearsal55.pulllist.app')` → `'rehearsal55'` (allowlist does not swallow) ✓; staging asymmetry noted (no live subdomain on staging — per §1.2). Seeded 2 catalog rows (`RHR-001`, `RHR-002`) in rehearsal tenant. Isolation probes: founding→rehearsal = 0 all 7 tables; rehearsal→founding = 0 all 7 tables; rehearsal own catalog via RLS = 2 ✓. Playwright 19/19 green with **three tenants present** (founding + rehearsal55 + Playwright synthetic `pw-8a9abc57`) — F15/F20 green; branding unit tests green; synthetic torn down by suite ✓. FK-ordered teardown executed (§4.1 delete order scoped to `699328d9-…`); GoTrue admin API delete failed (invalid key) — worked around with direct `DELETE FROM auth.users` via SQL Editor. Post-teardown: `rehearsal_tenant_rows=0`, `rehearsal_profiles=0`, `rehearsal_auth_users=0`; founding intact (109 profiles). **S2 paused: no real tenant ready at execution — waiting for operator-supplied inputs.** |
| 2026-06-19 | S2 | Green | Real tenant 2 created via `register-tenant` prod → 200: tenant_id `6f6ef2c3-…`, admin_user_id `49a1db65-…`, slug `comicstore`; webhook_secret saved to local scratch file only (not in repo). Branding `{"primary_color":"#00FF00","display_name":"Comic Store"}` confirmed in DB SELECT ✓. `comicstore.pulllist.app` custom domain Active, TLS issued, HTTPS 200 ✓. `resolve_tenant_by_slug('comicstore')` RPC → correct tenant_id + branding ✓; `resolve_tenant_by_slug('rjbookstop')` → founding unaffected ✓. Admin sign-in at `comicstore.pulllist.app` confirmed (cross-origin re-auth expected — sessions are origin-scoped); admin surface empty, 0 customers, no founding data visible ✓. MailerLite webhook **deferred** — pilot customers via `create-paper-customer`/admin invite; founding MailerLite unchanged ✓. Founding write-smoke: reserve + cancel on `pulllist.app` → `smoke_rows = 0`, founding `tenant_id` confirmed ✓. Note: "Create Account" link (customer self-reg UI, bypassing MailerLite webhook) noted as Phase 6 follow-on idea. **No secret values recorded.** |
| 2026-06-19 | S3 | Green | Pilot catalog seeded: 2 rows (TCS-001/TCS-002, `Lunar`, `Standard`, `2026-06`) in tenant 2 ✓. Cross-tenant data isolation matrix both directions, 7 tables: all = 0 ✓; `t2_catalog_visible_to_founding = 0` explicit check ✓. `get_popular_series('2026-06')` = 0 via SQL Editor (no June 2026 preorders yet; F20 dormant — no cross-tenant data to leak at this stage; will activate when tenant 2 has preorders in the same catalog month as founding) ✓. `register-customer` with tenant-2 webhook secret → `user_profiles.tenant_id = 6f6ef2c3-…`, `status = pending` ✓; probe row deleted ✓. Founding register-customer path unchanged from 5.4 (no source edits in 5.5; founding write-smoke S2 green) ✓. Branding-by-host: `comicstore.pulllist.app` → `#00FF00`/Comic Store; `pulllist.app` → founding `{}` (re-confirmed from S2) ✓. Playwright 19/19 green (F15/F20 tenant-isolation specs + branding unit tests green; synthetic tenant created + torn down by suite) ✓; import regression 5/5 ✓. Tenants table: exactly 2 rows (founding + comicstore) ✓. **Operational note:** `create-paper-customer` and `invite-customer` EFs have F34 residual partial tenant-awareness (write to `FOUNDING_TENANT_ID` regardless of calling admin's tenant); do NOT use from the Comic Store admin dashboard during pilot — customers would land in the founding tenant. |
| 2026-06-20 | S4 | In progress | Soak opened; `docs/phase-5.5-soak-log.md` created; close gate = one full import cycle + post-import isolation re-verify = 0 both directions; tenant 2 (`comicstore`) pilot/seeded throughout; monthly import expected early July 2026. |
| 2026-07-15 | S4 | Green (Closed) | Import cycle 2026-07-08→10 elapsed. Post-import isolation re-verify: `comicstore` byte-identical to pre-import baseline (profiles=1, catalog=2, preorders=0); `rjbookstop` normal one-month growth (catalog +1147, preorders +475, reservation_history +247 archived); orphan check 0/0/0 across catalog/preorders/user_profiles; `total_tenants=2`. Founding Playwright 19/19 green (run directly via `npx playwright test`; `run-smoke.ps1` wrapper truncated output for an unrelated reason, not investigated further — out of scope). Founding write-smoke confirmed clean on prod (Rick). No customer-reported regressions across the 2026-06-20→2026-07-15 span. **Soak PASSED** — full detail in `docs/phase-5.5-soak-log.md` § S4 close. **Also this session:** corrected a stale F34-residual claim (`create-paper-customer`/`invite-customer` do NOT hard-pin to founding — fixed 2026-05-10; the S3-era caution was reading pre-fix doc prose) across `technical-reference.md` §11/§13, the soak log, and `docs/tenant-onboarding-runbook.md` (commit `384b5ba`). |
| 2026-06-20 | S5 | Green | `docs/tenant-onboarding-runbook.md` written — repeatable tenant-N+1 onboarding (gather inputs → `register-tenant` → Cloudflare custom domain → branding → webhook → admin handoff → isolation spot-check → real-customer go-live incl. F72 email-branding decision + one-way rollback note). F34 residual (`create-paper-customer`/`invite-customer` write to founding) noted as operational constraint. Written ahead of S4 close (doc-only; no S4 dependency). |
| | S6 | | |
