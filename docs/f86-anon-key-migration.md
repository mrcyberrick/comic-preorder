...
- **Step 3 complete.** Branched `chore/f86-prod-publishable-key` off `main` (commit `f608d18`, Rick's edit). Diff shape verified by agent (value never read): `git diff main --stat` → exactly one file (`config.js`), 1 insertion/1 deletion; diff content confirmed only `SUPABASE_ANON_KEY`'s value changed, same variable/quoting, old prefix `eyJhbGciOiJIUzI1Ni…` → new prefix `sb_publishable_pFa…` (18-char prefix shown, remainder redacted). **V4 green.** PR [#80](https://github.com/mrcyberrick/comic-preorder/pull/80) opened → `main`, Cloudflare Pages check passed, merged (`415f742`, 2026-07-16T12:58:42Z) after explicit confirmation (merging to `main` triggers an immediate prod deploy — treated as its own checkpoint separate from the config.js edit itself). Live deploy confirmed propagated (`pulllist.app/config.js` serving `sb_publishable_pFa…`). **V5 green** — Rick verified manually (Playwright is staging-only by design, never runs against prod): pulllist.app loads, login works, catalog renders, reserve→My List→cancel write-smoke all correct; `comicstore.pulllist.app` (tenant 2, shares this `config.js`) also loads correctly. **The legacy anon key is still enabled at this point** (by design — Step 5 hasn't run), so rollback remains a one-commit revert if anything surfaces later.
- **Next: Step 4** — quiet window. Let prod run on the publishable key through **at least one weekly shipment cycle (a Wednesday)** with no anon-key-related errors before Step 5 (the prod toggle flip). Also: confirm Rick's `SUPABASE_SERVICE_KEY_PROD` rotation (filed above) completed during this window if not already done.
