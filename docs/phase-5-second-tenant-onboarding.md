...
- [x] Hosting fully migrated; GitHub Pages serving retired; deployment workflow in `CLAUDE.md` rewritten for the new host — active serving moved to Cloudflare Pages at 5.1 (2026-06-14); this criterion is about *serving*, satisfied since 5.1. Separately: the legacy `mrcyberrick.github.io`/`mrcyberrick.us` GH Pages copies were kept **warm as a rollback surface "until 5.5 closes"** (`CLAUDE.md`) — neither this plan nor the 5.5 plan stated an explicit retirement disposition for that surface once 5.5 closed, and it was flagged for Rick rather than retired unilaterally. **That disposition now exists — Rick, 2026-08-29: keep it warm, because it is free** (`mrcyberrick/comic-preorder` is a public repo, so GitHub Pages costs nothing; the `mrcyberrick.us` registration is independent). **This open item is closed.** One caveat, measured 2026-08-29 rather than assumed: the surface auto-deploys from `main` and renders as the *apex* front door (no tenant slug in `mrcyberrick.us`), so it is a live mirror of production rather than a frozen rollback snapshot — magic-link sign-in still works there, but a rollback would lose the branded first impression. See `CLAUDE.md` § Project Overview
