...
| F52 | 5 of 8 Edge Functions not committed to repo (`approve-customer`, `claim-paper-customer`, `notify-customers`, `reset-password`, `send-my-list`). 4.6 plan requires tagged-commit redeploy of all 8 — not currently executable | C1.B source location investigation | Defer to 4.6: add prerequisite task to commit all 8 EFs to repo before cutover tag | Sources confirmed via CLI download during B5. Staging-deployed source ≠ Downloads/prod for approve-customer (different BASE_URL, JWT comment). |
...
| 2026-05-26 | C2 SQL | [pending user paste] | |
